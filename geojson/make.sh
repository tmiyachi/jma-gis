...
$MSCMD ${GEODIR}/tmp.geojson -simplify ${SIMPLIFY_PERCENT} -o - | \
$MSCMD -i - -filter-slivers min-area=${SIMPLIFY_MINAREA} -o - | \
$MSCMD -i - -clean -o ${GEODIR}/tmp.geojson force

# フィールドを追加
echo -e "\e[1;33mJoin xml code table to fields...\e[0;m"
$MSCMD ${GEODIR}/tmp.geojson -each 'citycode=regioncode, delete regioncode' -o - | \
$MSCMD -i - -join ${JMACODE} keys=citycode,citycode string-fields=* -o ${GEODIR}/city.geojson force
